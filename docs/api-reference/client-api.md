...
1.  _array_ `$settings` — (Required) Configuration. Includes the following:
    ...
    -   _array_ `credentials` — (Required if not using the [Implicit
        Workflow](../implicit-workflow.md)) Application's Hawk credentials,
        which include the following:

        -   _string_ `key` — Secret key for the application

        -   _string_ `algorithm` — Algorithm to be used for HMAC. The value must
            be either `sha256` (recommended) or `sha1`.
...
1.  _string_ `$flow` — (Optional) Type of Oz flow to use to attempt to retrieve
    ...
    -   `auto` — (Default) Automatically determine the flow being used based on
        ...
        [constructor](#connection-constructor). If the application
        credentials are set, then the  [User Credentials](../user-credentials-workflow.md)
        flow will be used. If the application credentials are NOT set, then the
